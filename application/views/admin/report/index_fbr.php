<?php
	...
	$report = $this->db->get('adt_report_imported_pl_fb');
	...
	<?php
		if(isset($_POST['kst_etadate']) || isset($_POST['exfctdate'])){
			?>
				<div class="col-sm-9">
					<br>
					<a href="<?=base_url('admin/xls_pilih_fbr?exfctdate='.$exfctdate.'&kst_etadate='.$kst_etadate.'&c_bpartner_id='.$c_bpartner_id);?>" class="btn btn-flat btn-success pull-right"><i class='fa fa-download'></i> DOWNLOAD</a>
				</div>
			<?php
		}
	?>
...
<table class="table table-striped table-bordered" id="tablesss">
	...
	?>
		<tr>
			...
			<td><?=$po->category;?></td>
			<td><?=$po->item;?></td>
			<td><?=($po->date_required == NULL) ? '-' : date('Y-m-d',strtotime($po->date_required));?></td>
