			...
			<tbody>
				<?php
					...
					?>
							...
							<?php
								if($b->is_locked == 't'){
									...
									echo "<a title='Print Barcode' target='_Blank' style='margin-right:10px' href='".base_url('label/label_fb_new/'.$b->no_packinglist).'/'.$b->kst_suratjalanvendor.'/'.$b->kst_invoicevendor.'/'.$b->kst_resi."'><i class='fa fa-qrcode'></i></a>";
										...
										echo "<a title='Print Barcode' target='_Blank' style='margin-right:10px' href='".base_url('label/label_acc_new/'.$b->no_packinglist).'/'.$b->kst_suratjalanvendor.'/'.$b->kst_invoicevendor.'/'.$b->kst_resi."'><i class='fa fa-qrcode'></i></a>";
									}	
								}else{
						...
						<!--/td>
						<td class="text-center"-->
							<a title="Delete Header" style='margin-right:10px' onclick="return confirm('Do you realy want to delete this Packing List?')" href='<?=base_url('data/isactive?token='.$this->session->userdata('session_id').'&no_packinglist='.$b->no_packinglist);?>'><i class='fa fa-trash'></i></a>
						<!--/td>
						<td class="text-center"-->
						<?php
							if($b->is_locked == 'f'){
								echo "<a onclick='return confirm(\"Do you relay want to Lock this Packing List?\")' href='".base_url('data/lock?no_packinglist='.$b->no_packinglist)."'><i class='fa fa-unlock'></i></a>";
							}else{
								echo "<a onclick='return confirm(\"Do you relay want to Lock this Packing List?\")' href='".base_url('data/unlock?no_packinglist='.$b->no_packinglist)."'><i class='fa fa-lock'></i></a>";
							}
						?>						
						</td>
						</tr>
